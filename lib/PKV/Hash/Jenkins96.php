...
final class Jenkins96 implements HashInterface
{
    ...
    public function compute(string $data): int
    {
        ...
        if ($i < $len) { $a = ($a + self::u32ord($data[$i++])) & 0xFFFFFFFF; }
        if ($i < $len) { $a = ($a + ((self::u32ord($data[$i++]) << 8)  & 0xFFFFFFFF)) & 0xFFFFFFFF; }
        if ($i < $len) { $a = ($a + ((self::u32ord($data[$i++]) << 16) & 0xFFFFFFFF)) & 0xFFFFFFFF; }
        if ($i < $len) { $a = ($a + ((self::u32ord($data[$i++]) << 24) & 0xFFFFFFFF)) & 0xFFFFFFFF; }

        if ($i < $len) { $b = ($b + self::u32ord($data[$i++])) & 0xFFFFFFFF; }
        if ($i < $len) { $b = ($b + ((self::u32ord($data[$i++]) << 8)  & 0xFFFFFFFF)) & 0xFFFFFFFF; }
        if ($i < $len) { $b = ($b + ((self::u32ord($data[$i++]) << 16) & 0xFFFFFFFF)) & 0xFFFFFFFF; }
        if ($i < $len) { $b = ($b + ((self::u32ord($data[$i++]) << 24) & 0xFFFFFFFF)) & 0xFFFFFFFF; }

        if ($i < $len) { $c = ($c + ((self::u32ord($data[$i++]) << 8)  & 0xFFFFFFFF)) & 0xFFFFFFFF; }
        if ($i < $len) { $c = ($c + ((self::u32ord($data[$i++]) << 16) & 0xFFFFFFFF)) & 0xFFFFFFFF; }
        if ($i < $len) { $c = ($c + ((self::u32ord($data[$i])  << 24) & 0xFFFFFFFF)) & 0xFFFFFFFF; }
